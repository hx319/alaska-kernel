...
echo "create gzpeite img.."
dd if=/dev/zero of=$IMAGE_PATH/gzpeite.img bs=1M count=12
mkfs.vfat $IMAGE_PATH/gzpeite.img
mkdir $IMAGE_PATH/tmp
sudo mount -o loop $IMAGE_PATH/gzpeite.img $IMAGE_PATH/tmp
cd kernel-5.10
cpp -E -Wp,-MD,arch/arm64/boot/dts/rockchip/.rk3562-gzpeite.dtsi.pre.tmp \
  -nostdinc -I./scripts/dtc/include-prefixes -undef -D__DTS__ -x assembler-with-cpp \
  -o arch/arm64/boot/dts/rockchip/.rk3562-gzpeite.dtsi.tmp \
  arch/arm64/boot/dts/rockchip/rk3562-gzpeite.dtsi
cd -
sudo mv kernel-5.10/arch/arm64/boot/dts/rockchip/.rk3562-gzpeite.dtsi.tmp $IMAGE_PATH/tmp/rk3562-gzpeite.dtsi
sleep 1
sudo umount $IMAGE_PATH/tmp || true
sudo umount $IMAGE_PATH/tmp || true
sudo umount $IMAGE_PATH/tmp || true
sudo umount $IMAGE_PATH/tmp || true
sudo umount $IMAGE_PATH/tmp || true
sudo umount $IMAGE_PATH/tmp || true
sleep 1
sudo rm -rf $IMAGE_PATH/tmp
rm -rf kernel-5.10/arch/arm64/boot/dts/rockchip/.rk3562-gzpeite.dtsi.pre.tmp

dd if=/dev/zero of=$IMAGE_PATH/logo_rk.img bs=1M count=128
mkfs.vfat $IMAGE_PATH/logo_rk.img
