...
static void openDevice(JNIEnv *env, jobject thiz)
{
    ...
    char video_name_tmp[64];
    int i;
    for(i=0; i<8; i++)
    {
      sprintf(video_name_tmp, "/dev/v4l-subdev%d", i);
      camFd = open(video_name_tmp, O_RDWR);
      if(camFd >= 0)
      {
        ioctl(camFd, VIDIOC_QUERYCAP, &cap);
        close(camFd);
        LOGE("open %s success, name=%s", video_name_tmp, cap.driver);
        if(strcmp((const char *)cap.driver, "tc35874x") == 0)
        {
         sprintf(video_name, "/dev/v4l-subdev%d", i);
         break;
        }
      }
    }
