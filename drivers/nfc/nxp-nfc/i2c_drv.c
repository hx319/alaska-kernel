...
static int nfc_detect(struct i2c_client *client)
{
	int retry_time = 5;
  ...
  while(retry_time-- > 0)
  {
    ...
    int i,ret;
  	...
  	msleep(50);
  	ret = i2c_master_recv(client, NCICoreReset, 3);
    printk(KERN_CONT "nfc_detect recv:");
  	for(i=0; i<ret; i++)
  	  printk(KERN_CONT " %02X", NCICoreReset[i]);
  	return 0;
  }
	return -1;
}
