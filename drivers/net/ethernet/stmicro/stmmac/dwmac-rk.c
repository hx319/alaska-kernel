...
static unsigned char pet_mac[ETH_ALEN] = {0};
static unsigned char globe_mac[ETH_ALEN * MAX_ETH] = {0};
...
static void rk_get_eth_addr(void *priv, unsigned char *addr)
{
  ...
  if ((ret != ETH_ALEN * MAX_ETH) || (!is_valid_ether_addr(&ethaddr[id * ETH_ALEN])))
  {
    unsigned int mac_0, mac_1;
    printk("%s: rk_vendor_read eth mac address failed (%d)", __func__, ret);

    mac_0 = (system_serial_high << 6) | (system_serial_high >> (26));
    mac_1 = (system_serial_low << 6) | (system_serial_low >> (26));

    ethaddr[0] = ((mac_0 >> 24) & 0xFF);
    ethaddr[1] = ((mac_0 >> 16) & 0xFF);
    ethaddr[2] = ((mac_0 >> 8) & 0xFF);
    ethaddr[3] = (mac_0 & 0xFF);
    ethaddr[4] = ((mac_1 >> 8) & 0xFF);
    ethaddr[5] = (mac_1 & 0xFF);
    ...
    ethaddr[8] = ethaddr[2];
    ethaddr[9] = ethaddr[3];
    ethaddr[10] = ethaddr[4];
    ethaddr[11] = ethaddr[5] + 1;

    printk("%s: generate cpuid eth mac address: %02x:%02x:%02x:%02x:%02x:%02x", __func__, ethaddr[0], ethaddr[1], ethaddr[2], ethaddr[3], ethaddr[4], ethaddr[5]);
    printk("%s: generate cpuid eth mac address: %02x:%02x:%02x:%02x:%02x:%02x", __func__, ethaddr[6], ethaddr[7], ethaddr[8], ethaddr[9], ethaddr[10], ethaddr[11]);
    memcpy(globe_mac, ethaddr, ETH_ALEN * MAX_ETH);
    memcpy(addr, &ethaddr[id * ETH_ALEN], ETH_ALEN);

    ret = rk_vendor_write(LAN_MAC_ID, ethaddr, ETH_ALEN * MAX_ETH);
    if (ret != 0)
    {
      printk("%s: rk_vendor_write eth mac address failed (%d)", __func__, ret);
      return;
    }
    else
      printk("%s: rk_vendor_write eth mac address ok", __func__);
  }
  else
  {
    printk("%s: rk_vendor_read eth mac %d address ok", __func__, id);
    memcpy(addr, &ethaddr[id * ETH_ALEN], ETH_ALEN);
    memcpy(globe_mac, ethaddr, ETH_ALEN * MAX_ETH);
  }
}

void write_mac_work(struct work_struct *work)
{
  int ret;
  struct delayed_work *write_mac_delayed_work = to_delayed_work(work);
  static int write_retry = 9;
  unsigned char ethaddr[ETH_ALEN * MAX_ETH] = {0};

  ret = rk_vendor_read(LAN_MAC_ID, ethaddr, ETH_ALEN * MAX_ETH);
  if((ret != ETH_ALEN * MAX_ETH) || (!is_valid_ether_addr(ethaddr)) || (!is_valid_ether_addr(&ethaddr[ETH_ALEN])))
  {
    ret = rk_vendor_write(LAN_MAC_ID, globe_mac, ETH_ALEN * MAX_ETH);
    if(ret != 0)
    {
      printk("%s: rk_vendor_write eth mac address failed (%d)", __func__, ret);
      if(write_retry >= 0)
        schedule_delayed_work(write_mac_delayed_work, msecs_to_jiffies(1000));
      write_retry--;
    }
    else
      printk("%s: rk_vendor_write eth mac address ok", __func__);
    ...
    printk("%s: rk_vendor_read eth mac address ok", __func__);
}
...
static int rk_gmac_probe(struct platform_device *pdev)
{
  ...
  rk_get_eth_addr(plat_dat->bsp_priv, pet_mac);
  stmmac_res.mac = pet_mac;
  INIT_DELAYED_WORK(&plat_dat->write_mac_delayed_work, write_mac_work);
  schedule_delayed_work(&plat_dat->write_mac_delayed_work, msecs_to_jiffies(1000));
