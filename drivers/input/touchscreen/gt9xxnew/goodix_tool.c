...
static u8 relation(u8 src, u8 dst, u8 rlt)
{
  ...
  case 1:
    ret = (src == dst) ? true : false;
    dev_dbg(&gt_client->dev, "equal:src:0x%02x  dst:0x%02x  ret:%d.", src, dst, (s32)ret);
    break;
  ...
  for (i = 0; i < cmd_head.times; i++) {
    ...
    if (true == relation(buf[GTP_ADDR_LENGTH],
     cmd_head.flag_val, cmd_head.flag_relation)) {
      dev_dbg(&gt_client->dev, "value at flag addr:0x%02x.", buf[GTP_ADDR_LENGTH]);
      dev_dbg(&gt_client->dev, "flag value:0x%02x.", cmd_head.flag_val);
      break;
    }
...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
  ...
  dev_dbg(&gt_client->dev, "[Flag]flag: %02X,addr: %02X%02X,value: %02X,relation: %02X", cmd_head.flag, cmd_head.flag_addr[0], cmd_head.flag_addr[1], cmd_head.flag_val, cmd_head.flag_relation);
  dev_dbg(&gt_client->dev, "[Retry]circle: %d,times: %d,retry: %d, delay: %d", (s32)cmd_head.circle, (s32)cmd_head.times, (s32)cmd_head.retry, (s32)cmd_head.delay);
  ...
  if (1 == cmd_head.wr) {
    ...
    if (tool_i2c_write(&cmd_head.data[GTP_ADDR_LENGTH - cmd_head.addr_len], cmd_head.data_len + cmd_head.addr_len) <= 0) {
...
ssize_t goodix_tool_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
  ...
  if (*ppos) {
     ...
     * dev_dbg(&gt_client->dev, "[PARAM]size: %d, *ppos: %d", size, (int)*ppos);
     * dev_dbg(&gt_client->dev, "[TOOL_READ]ADB call again, return it.");
     */
    *ppos = 0;
    return 0;
  }
