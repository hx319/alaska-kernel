...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
  ...
  if (cfg->length < GTP_CONFIG_MIN_LENGTH) {
    dev_err(&ts->client->dev, "No valid config with sensor_ID(%d) ", sensor_id);
  ...
  if (ret == SUCCESS) {
    dev_dbg(&ts->client->dev, "CFG_GROUP%d Config Version: %d, IC Config Version: %d", sensor_id, cfg->data[GTP_ADDR_LENGTH], opr_buf[0]);
    ...
    dev_err(&ts->client->dev, "Failed to get ic config version!No config sent!");
    return -EPERM;
  }
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr, u8 *msg, s32 len)
{
  ...
  if (i >= 5) {
    dev_err(&client->dev, "Read data from 0x%02x%02x failed!", msg[0], msg[1]);
    return FAIL;
  }
...
static u8 gup_set_ic_msg(struct i2c_client *client, u16 addr, u8 val)
{
  ...
  if (i >= 5) {
    dev_err(&client->dev, "Set data to 0x%02x%02x failed!", msg[0], msg[1]);
    return FAIL;
  }
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
  ...
  for (i = 0; i < 4; i++)
    ...
    dev_dbg(&client->dev, "IC Hardware info:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1], update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
  ...
  dev_dbg(&client->dev, "IC force update flag:0x%x", update_msg.fw_damaged);
  ...
  if (update_msg.ic_fw_msg.pid[0] != 0) {
    if (!memcmp(update_msg.ic_fw_msg.pid, "9111", 4)) {
      dev_dbg(&client->dev, "IC Mapping Product id:%s", update_msg.ic_fw_msg.pid);
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
  ...
  while (retry++ < 200) {
    ...
    if (ret <= 0) {
      dev_dbg(&client->dev, "Hold ss51 & dsp I2C error,retry:%d", retry);
      continue;
    }
    ...
    dev_dbg(&client->dev, "Hold ss51 & dsp confirm 0x4180 failed,value:%d", rd_buf[GTP_ADDR_LENGTH]);
  }
  if (retry >= 200) {
...
void gup_leave_update_mode(struct i2c_client *client)
{
  ...
  if (ts->pdata->int_sync && ts->pinctrl.pinctrl)
    pinctrl_select_state(ts->pinctrl.pinctrl, ts->pinctrl.int_input);
...
static u8 gup_enter_update_judge(struct i2c_client *client, struct st_fw_head *fw_head)
{
  u16 u16_tmp;
  s32 i = 0;
  u32 fw_len = 0;
  ...
  if (update_msg.fw_total_len != fw_len) {
    dev_err(&client->dev, "Inconsistent firmware size, Update aborted!");
    dev_err(&client->dev, " Default size: %d(%dK), actual size: %d(%dK)", fw_len, fw_len/1024, update_msg.fw_total_len, update_msg.fw_total_len/1024);
    return FAIL;
  }
  dev_info(&client->dev, "Firmware length:%d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
...
static int gup_update_config(struct i2c_client *client)
{
  ...
  file_cfg_len = gtp_ascii_to_array(fw_cfg->data, fw_cfg->size, &file_config[GTP_ADDR_LENGTH]);
  if (file_cfg_len < 0) {
...
static u8 gup_get_update_file(struct i2c_client *client, struct st_fw_head *fw_head, u8 *path)
{
  s32 ret = 0;
  s32 i = 0;
  s32 fw_checksum = 0;
  ...
  dev_info(&client->dev, "FW File: %s size=%zu", path, update_msg.fw->size);
  ...
  if (update_msg.fw_total_len < FW_HEAD_LENGTH + FW_SECTION_LENGTH * 4 + FW_DSP_ISP_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH) {
    dev_err(&client->dev, "INVALID bin file(size: %d), update aborted.", update_msg.fw_total_len);
    goto invalied_fw;
  }

  update_msg.fw_total_len -= FW_HEAD_LENGTH;

  dev_dbg(&client->dev, "Bin firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
  ...
  for (i = 0; i < update_msg.fw_total_len; i += 2) {
    ...
    temp = (update_msg.fw_data[FW_HEAD_LENGTH + i] << 8) + update_msg.fw_data[FW_HEAD_LENGTH + i + 1];
    fw_checksum += temp;
  }
...
static u8 gup_burn_proc(struct i2c_client *client, u8 *burn_buf, u16 start_addr, u16 total_length)
{
  ...
  dev_dbg(&client->dev, "Begin burn %dk data to addr 0x%x", total_length / 1024, start_addr);
  while (burn_length < total_length) {
    dev_dbg(&client->dev, "B/T:%04d/%04d", burn_length, total_length);
    ...
    for (retry = 0; retry < MAX_FRAME_CHECK_TIME; retry++) {
      ...
      if (ret <= 0) {
        dev_err(&client->dev, "Read back frame data i2c error.");
        continue;
      }
      if (memcmp(&wr_buf[GTP_ADDR_LENGTH],
        &rd_buf[GTP_ADDR_LENGTH], frame_length)) {
        dev_err(&client->dev, "Check frame data fail,not equal.");
        ...
        /* dev_dbg(&client->dev, "Check frame data success."); */
        break;
      }
    }
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
  if (!update_msg.fw_data || update_msg.fw_total_len < offset + length) {
    dev_err(&i2c_connect_client->dev, "cannot load section data. fw_len=%d read end=%d\n", update_msg.fw_total_len, FW_HEAD_LENGTH + offset + length);
    return FAIL;
  }
  ...
  if (SEEK_SET == set_or_end) {
    ...
    /* seek end */
    memcpy(buf, &update_msg.fw_data[update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
  }
...
static u8 gup_recall_check(struct i2c_client *client, u8 *chk_src, u16 start_rd_addr, u16 chk_length)
{
  ...
  dev_dbg(&client->dev, "Recall check %dk firmware success.", (chk_length/1024));
...
static u8 gup_burn_fw_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u8 bank_cmd)
{
  ...
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_section]select bank %d fail.", (bank_cmd >> 4)&0x0F);
    return FAIL;
  }
  ...
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_section]enable accessing code fail.");
    return FAIL;
  }
  ...
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_section]hold ss51 & release dsp fail.");
    return FAIL;
  }
  /* must delay */
  ...
  do {
    ...
    if (ret <= 0) {
      dev_err(&client->dev, "[burn_fw_section]Get burn state fail");
      return FAIL;
    }
    usleep_range(10000, 11000);
  /* dev_dbg(&client->dev, "[burn_fw_section]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]); */
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_section]recall check %dk firmware fail.", FW_SECTION_LENGTH / 1024);
    return FAIL;
  }
  ...
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_section]disable accessing code fail.");
    return FAIL;
  }
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
  ...
  while (retry++ < 5) {
    ...
    if (fw_dsp_isp == NULL) {
      ...
      dev_info(&client->dev, "[burn_dsp_isp]Alloc %dk byte memory success.", FW_DSP_ISP_LENGTH / 1024);
      break;
    }
  }
  if (retry >= 5) {
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_dsp_isp]load firmware dsp_isp fail.");
    goto exit_burn_dsp_isp;
  }

  /* step3:disable wdt,clear cache enable */
  dev_dbg(&client->dev, "[burn_dsp_isp]step3:disable wdt,clear cache enable");
  ...
  dev_dbg(&client->dev, "[burn_dsp_isp]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
  ...
  while (retry++ < 5) {
    ...
    if (fw_ss51 == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_ss51]Alloc %dk byte memory success.", (FW_SECTION_LENGTH / 1024));
      break;
    }
  }
  if (retry >= 5) {
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]burn ss51 firmware section 1 fail.");
    goto exit_burn_fw_ss51;
  }

  /* step5:load ss51 firmware section 2 file data */
  dev_dbg(&client->dev, "[burn_fw_ss51]step5:load ss51 firmware section 2 file data");
  ret = gup_load_section_file(fw_ss51, FW_SECTION_LENGTH,
            FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]load ss51 firmware section 2 fail.");
    goto exit_burn_fw_ss51;
  }
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]burn ss51 firmware section 2 fail.");
    goto exit_burn_fw_ss51;
  }

  /* step7:load ss51 firmware section 3 file data */
  dev_dbg(&client->dev, "[burn_fw_ss51]step7:load ss51 firmware section 3 file data");
  ret = gup_load_section_file(fw_ss51, 2 * FW_SECTION_LENGTH, FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]load ss51 firmware section 3 fail.");
    goto exit_burn_fw_ss51;
  }

  /* step8:burn ss51 firmware section 3 */
  dev_dbg(&client->dev, "[burn_fw_ss51]step8:burn ss51 firmware section 3");
  ret = gup_burn_fw_section(client, fw_ss51, 0xC000, 0x13);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]burn ss51 firmware section 3 fail.");
    goto exit_burn_fw_ss51;
  }

  /* step9:load ss51 firmware section 4 file data */
  dev_dbg(&client->dev, "[burn_fw_ss51]step9:load ss51 firmware section 4 file data");
  ret = gup_load_section_file(fw_ss51, 3 * FW_SECTION_LENGTH, FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]load ss51 firmware section 4 fail.");
    goto exit_burn_fw_ss51;
  }
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_ss51]burn ss51 firmware section 4 fail.");
    goto exit_burn_fw_ss51;
  }

  update_msg.fw_burned_len += (FW_SECTION_LENGTH*4);
  dev_dbg(&client->dev, "[burn_fw_ss51]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
  ...
  while (retry++ < 5) {
    ...
    if (fw_dsp == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_dsp]Alloc %dk byte memory success.", FW_SECTION_LENGTH / 1024);
      break;
    }
  }
  if (retry >= 5) {
  ...
  dev_dbg(&client->dev, "[burn_fw_dsp]step8:send burn cmd to move data to flash from sram");
  ...
  do {
    ...
    usleep_range(10000, 11000);
  /* dev_dbg(&client->dev, "[burn_fw_dsp]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]); */
  } while (rd_buf[GTP_ADDR_LENGTH]);
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_dsp]recall check 4k dsp firmware fail.");
    goto exit_burn_fw_dsp;
  }

  update_msg.fw_burned_len += FW_DSP_LENGTH;
  dev_dbg(&client->dev, "[burn_fw_dsp]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
  ...
  while (retry++ < 5) {
    ...
    if (fw_boot == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_boot]Alloc %dk byte memory success.",
        FW_BOOT_LENGTH / 1024);
      break;
    }
  }
  ...
  /* step7:send burn cmd to move data to flash from sram */
  dev_dbg(&client->dev, "[burn_fw_boot]step7:send burn cmd to move data to flash from sram");
  ...
  do {
    ...
    usleep_range(10000, 11000);
  /* dev_dbg(&client->dev, "[burn_fw_boot]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]); */
  } while (rd_buf[GTP_ADDR_LENGTH]);

  /* step8:recall check 2k bootloader firmware */
  dev_dbg(&client->dev, "[burn_fw_boot]step8:recall check 2k bootloader firmware");
  ret = gup_recall_check(client, fw_boot, 0x9000, FW_BOOT_LENGTH);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_boot]recall check 2k bootcode firmware fail");
    goto exit_burn_fw_boot;
  }

  update_msg.fw_burned_len += FW_BOOT_LENGTH;
  dev_dbg(&client->dev, "[burn_fw_boot]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
  ...
  while (retry++ < 5) {
    ...
    if (fw_boot_isp == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_boot_isp]Alloc %dk byte memory success.", (FW_BOOT_ISP_LENGTH/1024));
      break;
    }
  }
  if (retry >= 5) {
  ...
  /* step2:load firmware bootloader */
  dev_dbg(&client->dev, "[burn_fw_boot_isp]step2:load firmware bootloader isp");
  ...
  /* step7:send burn cmd to move data to flash from sram */
  dev_dbg(&client->dev, "[burn_fw_boot_isp]step8:send burn cmd to move data to flash from sram");
  ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, 0x07);
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_boot_isp]send burn cmd fail.");
    goto exit_burn_fw_boot_isp;
  }
  dev_dbg(&client->dev, "[burn_fw_boot_isp]Wait for the burn is complete......");
  do {
    ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
    if (ret <= 0) {
      dev_err(&client->dev, "[burn_fw_boot_isp]Get burn state fail");
      goto exit_burn_fw_boot_isp;
    }
    usleep_range(10000, 11000);
  /* dev_dbg(&client->dev, "[burn_fw_boot_isp]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]); */
  } while (rd_buf[GTP_ADDR_LENGTH]);

  /* step8:recall check 2k bootload_isp firmware */
  dev_dbg(&client->dev, "[burn_fw_boot_isp]step9:recall check 2k bootloader firmware");
  ret = gup_recall_check(client, fw_boot_isp, 0x9000, FW_BOOT_ISP_LENGTH);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_boot_isp]recall check 2k bootcode_isp firmware fail.");
    goto exit_burn_fw_boot_isp;
  }

  update_msg.fw_burned_len += FW_BOOT_ISP_LENGTH;
  dev_dbg(&client->dev, "[burn_fw_boot_isp]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
  ...
  while (retry++ < 5) {
    ...
    if (fw_link == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_link]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
      break;
    }
  }
  if (retry >= 5) {
  ...
  if (FAIL == ret) {
    dev_err(&client->dev,"[burn_fw_link]load firmware link section 1 fail.");
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_link]burn link firmware section 1 fail.");
    goto exit_burn_fw_link;
  }

  /* step4:load link firmware section 2 file data */
  dev_dbg(&client->dev, "[burn_fw_link]step4:load link firmware section 2 file data");
  offset += FW_SECTION_LENGTH;
  ret = gup_load_section_file(fw_link, offset, FW_GLINK_LENGTH - FW_SECTION_LENGTH, SEEK_SET);

  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_link]load link firmware section 2 fail.");
    goto exit_burn_fw_link;
  }
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_link]burn link firmware section 2 fail.");
    goto exit_burn_fw_link;
  }

  update_msg.fw_burned_len += FW_GLINK_LENGTH;
  dev_dbg(&client->dev, "[burn_fw_link]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_gwake_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u32 len, u8 bank_cmd)
{
  ...
  ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x04);
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_app_section]hold ss51 & release dsp fail.");
    return FAIL;
  }
  /* must delay */
  usleep_range(1000, 2000);
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_app_section]burn fw_section fail.");
    return FAIL;
  }
  ...
  ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, bank_cmd&0x0F);
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_app_section]send burn cmd fail.");
    return FAIL;
  }
  dev_dbg(&client->dev, "[burn_fw_section]Wait for the burn is complete......");
  do {
    ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
    if (ret <= 0) {
      dev_err(&client->dev, "[burn_fw_app_section]Get burn state fail");
      return FAIL;
    }
    usleep_range(10000, 11000);
  /* dev_dbg(&client->dev, "[burn_fw_app_section]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]); */
  } while (rd_buf[GTP_ADDR_LENGTH]);

  /* step7:recall fw section */
  ret = gup_recall_check(client, fw_section, start_addr, len);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_app_section]recall check %dk firmware fail.", len/1024);
    return FAIL;
  }
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
  ...
  kzalloc(FW_SECTION_LENGTH, GFP_KERNEL);
    if (fw_gwake == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_gwake]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
      break;
    }
  }
  if (retry >= 5) {
  ...
  dev_dbg(&client->dev, "[burn_fw_gwake]step2:load app_code firmware section 1 file data");
  ret = gup_load_section_file(fw_gwake, start_index, FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]load app_code firmware section 1 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step3:burn app_code firmware section 1 */
  dev_dbg(&client->dev, "[burn_fw_gwake]step3:burn app_code firmware section 1");
  ret = gup_burn_fw_gwake_section(client, fw_gwake, 0x9000, FW_SECTION_LENGTH, 0x3A);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]burn app_code firmware section 1 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step5:load app_code firmware section 2 file data */
  dev_dbg(&client->dev, "[burn_fw_gwake]step5:load app_code firmware section 2 file data");
  ret = gup_load_section_file(fw_gwake, start_index+FW_SECTION_LENGTH, FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]load app_code firmware section 2 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step6:burn app_code firmware section 2 */
  dev_dbg(&client->dev, "[burn_fw_gwake]step6:burn app_code firmware section 2");
  ret = gup_burn_fw_gwake_section(client, fw_gwake, 0x9000, FW_SECTION_LENGTH, 0x3B);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]burn app_code firmware section 2 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step7:load app_code firmware section 3 file data */
  dev_dbg(&client->dev, "[burn_fw_gwake]step7:load app_code firmware section 3 file data");
  ret = gup_load_section_file(fw_gwake, start_index + 2 * FW_SECTION_LENGTH, FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]load app_code firmware section 3 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step8:burn app_code firmware section 3 */
  dev_dbg(&client->dev, "[burn_fw_gwake]step8:burn app_code firmware section 3");
  ret = gup_burn_fw_gwake_section(client, fw_gwake, 0x9000, FW_SECTION_LENGTH, 0x3C);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]burn app_code firmware section 3 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step9:load app_code firmware section 4 file data */
  dev_dbg(&client->dev, "[burn_fw_gwake]step9:load app_code firmware section 4 file data");
  ret = gup_load_section_file(fw_gwake, start_index + 3 * FW_SECTION_LENGTH, FW_SECTION_LENGTH, SEEK_SET);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]load app_code firmware section 4 fail.");
    goto exit_burn_fw_gwake;
  }

  /* step10:burn app_code firmware section 4 */
  dev_dbg(&client->dev, "[burn_fw_gwake]step10:burn app_code firmware section 4");
  ret = gup_burn_fw_gwake_section(client, fw_gwake, 0x9000, FW_SECTION_LENGTH, 0x3D);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_gwake]burn app_code firmware section 4 fail.");
    goto exit_burn_fw_gwake;
  }

  /* update_msg.fw_burned_len += FW_GWAKE_LENGTH; */
  dev_dbg(&client->dev, "[burn_fw_gwake]Burned length:%d", update_msg.fw_burned_len);
  ret = SUCCESS;
...
static u8 gup_burn_fw_finish(struct i2c_client *client)
{
  ...
  dev_info(&client->dev, "[burn_fw_finish]burn first 8K of ss51 and finish update.");
  /* step1:alloc memory */
  ...
  while (retry++ < 5) {
    ...
    if (fw_ss51 == NULL) {
      ...
      dev_dbg(&client->dev, "[burn_fw_finish]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
      break;
    }
  }
  if (retry >= 5) {
  ...
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_finish]load ss51 firmware section 1 fail.");
  ...
  ret = gup_burn_fw_section(client, fw_ss51, 0xC000, 0x01);
  if (FAIL == ret) {
    dev_err(&client->dev, "[burn_fw_finish]burn ss51 firmware section 1 fail.");
    goto exit_burn_fw_finish;
  }
  ...
  if (ret <= 0) {
    dev_err(&client->dev, "[burn_fw_finish]release ss51 & hold dsp fail.");
    goto exit_burn_fw_finish;
  }
...
s32 gup_update_proc(void *dir)
{
  ...
  if (FAIL == ret) {
    dev_err(&ts->client->dev, "Failed get valied firmware data\n");
  ...
  if (ts->force_update || dir) {
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]Doesn't meet update condition\n");
      goto file_fail;
    }
  }
  ...
  while (retry++ < 5) {
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn app_code firmware fail.");
      continue;
    }
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn ss51 firmware fail.");
      continue;
    }
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn dsp firmware fail.");
      continue;
    }
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn bootloader firmware fail.");
      continue;
    }
    show_len = 60;
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn boot_isp firmware fail.");
      continue;
    }
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn link firmware fail.");
      continue;
    }
    ...
    if (FAIL == ret) {
      dev_err(&ts->client->dev, "[update_proc]burn finish fail.");
      continue;
    }
    show_len = 90;
  ...
  if (retry >= 5) {
    dev_err(&ts->client->dev, "[update_proc]retry timeout,UPDATE FAIL.");
    update_ret = FAIL;
  } else {
    update_ret = SUCCESS;
  }
...
update_fail:
  ...
  if (SUCCESS == update_ret) {
    dev_info(&ts->client->dev, "firmware error auto update, resent config!\n");
    gup_init_panel(ts);
  }
