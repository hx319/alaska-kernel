...
extern int found_tp;
static unsigned char irq_flag_table[4] = {IRQ_TYPE_LEVEL_HIGH, IRQ_TYPE_LEVEL_LOW, IRQ_TYPE_LEVEL_LOW, IRQ_TYPE_LEVEL_HIGH};
...
int gtp_i2c_read(struct i2c_client *client, u8 *buf, int len)
{
  ...
  if (likely(len < sizeof(get_buf))) {
    ...
    msgs[1].buf = kzalloc(len > I2C_MAX_TRANSFER_SIZE ? I2C_MAX_TRANSFER_SIZE : len, GFP_KERNEL);
    if (!msgs[1].buf)
      return -ENOMEM;
  }
  ...
  while (pos != len) {
    ...
    if (unlikely(retry == RETRY_MAX_TIMES)) {
      dev_err(&client->dev, "I2c read failed,dev:%02x,reg:%04x,size:%u\n", client->addr, address, len);
      r = -EAGAIN;
      goto read_exit;
    }
  }
...

{
  ...
  if (likely(len < sizeof(put_buf))) {
    ...
    msg.buf = kmalloc(len > I2C_MAX_TRANSFER_SIZE ? I2C_MAX_TRANSFER_SIZE : len, GFP_KERNEL);
    if (!msg.buf)
      return -ENOMEM;
  }
  ...
  while (pos != len) {
    ...
    if (unlikely(retry == RETRY_MAX_TIMES)) {
      dev_err(&client->dev, "I2c write failed,dev:%02x,reg:%04x,size:%u\n", client->addr, address, len);
      r = -EAGAIN;
      goto write_exit;
    }
  }
...
s32 gtp_i2c_read_dbl_check(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
  ...
  if (len + 2 > sizeof(buf)) {
    dev_warn(&client->dev, "%s, only support length less then %zu\n", __func__, sizeof(buf) - 2);
    return FAIL;
  }
  while (retry++ < 3) {
    memset(buf, 0xAA, 16);
...
s32 gtp_send_cfg(struct i2c_client *client)
{
  ...
  if (!cfg->length || !ts->pdata->driver_send_cfg) {
    dev_info(&ts->client->dev, "No config data or error occurred in panel_init\n");
    return 0;
  }
  ...
  for (retry = 0; retry < RETRY_MAX_TIMES; retry++) {
    ret = gtp_i2c_write(client, cfg->data, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
    if (ret > 0)
      break;
  }
...
static void gtp_type_a_report(struct goodix_ts_data *ts, u8 touch_num, struct goodix_point_t *points)
{
  int i;
  u16 cur_touch = 0;
...
static void gtp_mt_slot_report(struct goodix_ts_data *ts, u8 touch_num, struct goodix_point_t *points)
{
  ...
  if((touch_num) && (ts->pdata->double_wakeup != 0) && (ts->pdata->suspend_flag != 0))
  {
    if(ts->pdata->suspend_flag == 1)
    {
      unsigned long double_time = (unsigned long)jiffies - old;
//      printk("jiffies-old = %lu\n", double_time);
      if(old == 0)         //old==0 为第一次触摸
      {
        old = (unsigned long)jiffies;
      }
      else if((double_time < 50) && (double_time > 8))
      {
        ts->pdata->suspend_flag = 2;
        old = 0;
//        printk("gtp KEY_POWER\n");
        input_report_key(ts->input_dev, KEY_POWER, 1);
        input_sync(ts->input_dev);
        input_report_key(ts->input_dev, KEY_POWER, 0);
        input_sync(ts->input_dev);
      }
      else if(double_time >= 50)
      {
        old = 0;
      }
      else
  ...
  for (i = 0; i < ts->pdata->max_touch_id; i++) {
    if (touch_num && i == points->id) {
      ...
      if (pre_pen_id == i) {
        ...
        /* valid id will < 10, so set id to 0xff to indicate a invalid state */
        pre_pen_id = 0xff;
      } else {
...
static void gtp_work_func(struct goodix_ts_data *ts)
{
  ...
  if (ts->pdata->slide_wakeup && test_bit(DOZE_MODE, &ts->flags)) {
    ...
    if (ret)
      dev_err(&ts->client->dev, "Failed handler gesture event %d\n", ret);
    return;
  }
  ...
  if (key_value & 0xf0 || pre_key & 0xf0) {
    ...
    for (i = 0; i < ts->pdata->key_nums; i++) {
      if ((pre_key | key_value) & (0x01 << i))
        input_report_key(ts->input_dev, ts->pdata->key_map[i], key_value & (0x01 << i));
    }
    input_sync(ts->input_dev);
    pre_key = key_value;
  }
...
void gtp_int_output(struct goodix_ts_data *ts, int level)
{
  ...
  if (level == 0) {
    if (ts->pinctrl.pinctrl)
      pinctrl_select_state(ts->pinctrl.pinctrl, ts->pinctrl.int_out_low);
    else if (gpio_is_valid(ts->pdata->irq_gpio))
      gpio_direction_output(ts->pdata->irq_gpio, 0);
    else
      dev_err(&ts->client->dev, "Failed set int pin output low\n");
  } else {
    if (ts->pinctrl.pinctrl)
      pinctrl_select_state(ts->pinctrl.pinctrl, ts->pinctrl.int_out_high);
    else if (gpio_is_valid(ts->pdata->irq_gpio))
      gpio_direction_output(ts->pdata->irq_gpio, 1);
    else
      dev_err(&ts->client->dev, "Failed set int pin output high\n");
  }
}
...
static int gtp_enter_doze(struct goodix_ts_data *ts)
{
  int ret = -1;
  int retry = 0;
  u8 i2c_control_buf[3] = { (u8)(GTP_REG_COMMAND >> 8), (u8)GTP_REG_COMMAND, 8 };
  ...
  while (retry++ < 5) {
    ...
    if (ret < 0) {
      dev_dbg(&ts->client->dev, "failed to set doze flag into 0x8046, %d\n", retry);
      continue;
    }
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data *ts)
{
  ...
  dev_dbg(&ts->client->dev, "Config Groups\' Lengths: %d, %d, %d, %d, %d, %d", cfg_info_len[0], cfg_info_len[1], cfg_info_len[2], cfg_info_len[3], cfg_info_len[4], cfg_info_len[5]);
#endif

  /* read sensor id */
  ret = gtp_i2c_read_dbl_check(ts->client, GTP_REG_SENSOR_ID, &sensor_id, 1);
  if (SUCCESS != ret || sensor_id >= 0x06) {
    dev_err(&ts->client->dev, "Failed get valid sensor_id(0x%02X), No Config Sent\n", sensor_id);
    return -EINVAL;
  }
  ...
  sensor_id = 0;
  ret = gtp_parse_dt_cfg(&ts->client->dev, &cfg->data[GTP_ADDR_LENGTH], &cfg->length, sensor_id);
  if (ret < 0) {
    dev_err(&ts->client->dev, "Failed to parse config data form device tree\n");
    cfg->length = 0;
    return -EPERM;
  }
#else
  ...
  memcpy(&cfg->data[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], cfg->length);
#endif

  if (cfg->length < GTP_CONFIG_MIN_LENGTH) {
    dev_err(&ts->client->dev, "Failed get valid config data with sensor id %d\n", sensor_id);
    cfg->length = 0;
    return -EPERM;
  }

  dev_info(&ts->client->dev, "GTP send cfg: %02X %02X %02X %02X %02X %02X %02X\n", cfg->data[2], cfg->data[3], cfg->data[4], cfg->data[5], cfg->data[6], cfg->data[7], cfg->data[8]);
  ts->pdata->abs_size_x = (cfg->data[4] << 8) + cfg->data[3];
  ts->pdata->abs_size_y = (cfg->data[6] << 8) + cfg->data[5];
  ts->pdata->max_touch_id = cfg->data[7];
  ts->pdata->irq_flags = irq_flag_table[cfg->data[8] & 0x03];
  dev_info(&ts->client->dev, "GTP send cfg: max_id=%d max_x=%d max_y=%d flag=0x%02X\n", ts->pdata->max_touch_id, ts->pdata->abs_size_x, ts->pdata->abs_size_y, ts->pdata->irq_flags);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
  ...
  if (!ts->pdata->driver_send_cfg) {
    ...
    ret = gtp_i2c_read(ts->client, cfg->data, cfg->length + GTP_ADDR_LENGTH);
    if (ret < 0)
      dev_err(&ts->client->dev, "Read origin Config Failed\n");

  dev_info(&ts->client->dev, "GTP read ic cfg: %02X %02X %02X %02X %02X %02X %02X\n", cfg->data[2], cfg->data[3], cfg->data[4], cfg->data[5], cfg->data[6], cfg->data[7], cfg->data[8]);
  ts->pdata->abs_size_x = (cfg->data[4] << 8) + cfg->data[3];
  ts->pdata->abs_size_y = (cfg->data[6] << 8) + cfg->data[5];
  ts->pdata->max_touch_id = cfg->data[7];
  ts->pdata->irq_flags = irq_flag_table[cfg->data[8] & 0x03];
  dev_info(&ts->client->dev, "GTP read ic cfg: max_id=%d max_x=%d max_y=%d flag=0x%02X\n", ts->pdata->max_touch_id, ts->pdata->abs_size_x, ts->pdata->abs_size_y, ts->pdata->irq_flags);
    return 0;
  }
  ...
  if (SUCCESS == ret) {
    if (opr_buf[0] != 0xBE) {
      ...
      dev_err(&ts->client->dev, "Firmware error, no config sent!\n");
      return -EINVAL;
    }
  }
  ...
  if (ret == SUCCESS) {
    dev_dbg(&ts->client->dev, "Config Version: %d; IC Config Version: %d\n", cfg->data[GTP_ADDR_LENGTH], opr_buf[0]);
...
static ssize_t gtp_config_read_proc(struct file *file, char __user *page,size_t size, loff_t *ppos)
{
  int i, ret;
  char *ptr;
  size_t data_len = 0;
  ...
  data_len += snprintf(ptr + data_len, 4096 - data_len, "====init value====\n");
  for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++) {
    data_len += snprintf(ptr + data_len, 4096 - data_len, "0x%02X ", cfg->data[i + 2]);
  ...
  ret = gtp_i2c_read(i2c_connect_client, temp_data, GTP_CONFIG_MAX_LENGTH + 2);
  if (ret < 0) {
    data_len += snprintf(ptr + data_len, 4096 - data_len, "Failed read real config data\n");
  } else {
    for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
      data_len += snprintf(ptr + data_len, 4096 - data_len, "0x%02X ", temp_data[i + 2]);
...
static ssize_t gtp_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
  u8 *temp_buf;
  u8 *file_config;
  int file_cfg_len;
  ...
  file_config = kzalloc(GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH, GFP_KERNEL);
  if (!file_config) {
...
static ssize_t gtp_productinfo_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  struct goodix_ts_data *data = dev_get_drvdata(dev);
...
static ssize_t gtp_drv_irq_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t gtp_drv_irq_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  struct goodix_ts_data *data = dev_get_drvdata(dev);

  return scnprintf(buf, PAGE_SIZE, "%s\n", test_bit(WORK_THREAD_ENABLED, &data->flags) ? "enabled" : "disabled");
}
static DEVICE_ATTR(drv_irq, 0664, gtp_drv_irq_show, gtp_drv_irq_store);

static ssize_t gtp_reset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
  struct goodix_ts_data *data = dev_get_drvdata(dev);
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
  ...
  gtp_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, 0664, NULL, &config_proc_ops);
  if (!gtp_config_proc)
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
  ...
  ret = gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID, &fw_info->sensor_id, 1);
  if (SUCCESS != ret || fw_info->sensor_id >= 0x06) {
    dev_err(&client->dev, "Failed get valid sensor_id(0x%02X), No Config Sent\n", fw_info->sensor_id);
...
static int gtp_pinctrl_init(struct goodix_ts_data *ts)
{
  ...
  pinctrl->default_sta = pinctrl_lookup_state(pinctrl->pinctrl, "default");
  ...
  pinctrl->int_out_high = pinctrl_lookup_state(pinctrl->pinctrl, "int-output-high");
  ...
  pinctrl->int_out_low = pinctrl_lookup_state(pinctrl->pinctrl, "int-output-low");
  if (IS_ERR_OR_NULL(pinctrl->int_out_low)) {
    dev_info(&ts->client->dev, "Failed get pinctrl state:output_low\n");
    goto exit_pinctrl_init;
  }

  pinctrl->int_input = pinctrl_lookup_state(pinctrl->pinctrl, "int-input");
  if (IS_ERR_OR_NULL(pinctrl->int_input)) {
    dev_info(&ts->client->dev, "Failed get pinctrl state:int-input\n");
    goto exit_pinctrl_init;
  }
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
  ...
  for (index = 0; index < ts->pdata->key_nums; index++)
    input_set_capability(ts->input_dev, EV_KEY, ts->pdata->key_map[index]);
  ...
  input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, ts->pdata->abs_size_x, 0, 0);
  input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, ts->pdata->abs_size_y, 0, 0);
  input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0, ts->pdata->max_touch_width, 0, 0);
  input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, ts->pdata->max_touch_pressure, 0, 0);
  input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, ts->pdata->max_touch_id, 0, 0);
  if (!ts->pdata->type_a_report) {
 ...
 */
#ifdef CONFIG_OF
static void gtp_parse_dt_coords(struct device *dev, struct goodix_ts_platform_data *pdata)
{
  struct device_node *np = dev->of_node;
  int ret;

  ret = of_property_read_u32(np, "touchscreen-max-id", &pdata->max_touch_id);
...
static int gtp_parse_dt(struct device *dev, struct goodix_ts_platform_data *pdata)
{
  ...
  if (ret) {
    dev_info(dev, "Failed get int-trigger-type from dts,set default\n");
  ...
  of_property_read_u32(np, "goodix,driver-send-cfg", &pdata->driver_send_cfg);
  ...
  if (pdata->revert_x)
    dev_info(dev, "revert-x enabled\n");

  pdata->revert_y = 0;
  of_property_read_u32(np, "goodix,revert-y", &pdata->revert_y);
  if (pdata->revert_y)
    dev_info(dev, "revert-y enabled\n");

  pdata->suspend_flag = 0;
  pdata->double_wakeup = 0;
  of_property_read_u32(np, "goodix,double-wakeup", &pdata->double_wakeup);
  if (pdata->double_wakeup)
    dev_info(dev, "double-wakeup enabled\n");
  ...
  of_property_read_u32(np, "goodix,type-a-report", &pdata->type_a_report);
  if (pdata->type_a_report)
    dev_info(dev, "type-a-report enabled\n");

  of_property_read_u32(np, "goodix,resume-in-workqueue", &pdata->resume_in_workqueue);
  if (pdata->resume_in_workqueue)
    dev_info(dev, "resume-in-workqueue enabled\n");

  of_property_read_u32(np, "goodix,power-off-sleep", &pdata->power_off_sleep);
  if (pdata->power_off_sleep)
    dev_info(dev, "power-off-sleep enabled\n");

  of_property_read_u32(np, "goodix,pen-suppress-finger", &pdata->pen_suppress_finger);
  ...
  if (prop) {
    ...
    ret = of_property_read_u32_array(np, "touchscreen-key-map", key_map, key_nums);
    if (ret) {
      dev_err(dev, "Unable to read key codes\n");
      pdata->key_nums = 0;
      memset(pdata->key_map, 0, MAX_KEY_NUMS * sizeof(pdata->key_map[0]));
    }
    pdata->key_nums = key_nums;
    memcpy(pdata->key_map, key_map, key_nums * sizeof(pdata->key_map[0]));
    dev_info(dev, "key-map is [%x %x %x %x]\n", pdata->key_map[0], pdata->key_map[1], pdata->key_map[2], pdata->key_map[3]);
  }
...
static int gtp_power_off(struct goodix_ts_data *ts)
{
  ...
  if (ts->vcc_i2c) {
    ...
    dev_info(&ts->client->dev, "Regulator vcc_i2c disabled\n");
  }
  ...
  if (ts->vdd_ana) {
    ...
    dev_info(&ts->client->dev, "Regulator vdd_ana disabled\n");
  }
  return ret;
...
static void gtp_suspend(struct goodix_ts_data *ts)
{
  ...
  if (test_bit(FW_UPDATE_RUNNING, &ts->flags)) {
    dev_warn(&ts->client->dev, "Fw upgrade in progress, can't go to suspend\n");
    return;
  }

  if(ts->pdata->double_wakeup)
  {
    ts->pdata->suspend_flag = 1;
    dev_info(&ts->client->dev, "double_wakeup suspend\n");
    return;
  }
...
static void gtp_resume(struct goodix_ts_data *ts)
{
  ...
  if (test_bit(FW_UPDATE_RUNNING, &ts->flags)) {
    dev_info(&ts->client->dev, "Fw upgrade in progress, can't do resume\n");
    return;
  }

  if(ts->pdata->double_wakeup)
  {
    ts->pdata->suspend_flag = 0;
    dev_info(&ts->client->dev, "double_wakeup resume\n");
    return;
  }
  ...
  if (ts->pdata->slide_wakeup && test_bit(DOZE_MODE, &ts->flags)) {
    ...
    if (ret) {
      ...
      ret = gtp_i2c_test(ts->client);
      if (ret)
        dev_warn(&ts->client->dev, "I2C communicate failed after power on\n");
    }
  } else {
    ret = gtp_wakeup_sleep(ts);
    if (ret)
      dev_warn(&ts->client->dev, "Failed wakeup from sleep mode\n");
  }
...
{
  int ret;
  u8 opr_buffer[3] = { (u8)(GTP_REG_ESD_CHECK >> 8), (u8)GTP_REG_ESD_CHECK, (u8)GTP_ESD_CHECK_VALUE };
...
static void gtp_esd_check_func(struct work_struct *work)
{
  ...
  struct goodix_ts_esd *ts_esd = container_of(dwork, struct goodix_ts_esd, delayed_work);
  struct goodix_ts_data *ts = container_of(ts_esd, struct goodix_ts_data, ts_esd);

  if (test_bit(SLEEP_MODE, &ts->flags) ||
      test_bit(FW_UPDATE_RUNNING, &ts->flags)) {
    dev_dbg(&ts->client->dev, "Esd cancled by power_suspend or fw_update!");
    return;
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    dev_dbg(&ts->client->dev, "[Esd]0x8040 = 0x%02X, 0x8041 = 0x%02X", esd_buf[2], esd_buf[3]);
    if (esd_buf[2] == (u8)GTP_ESD_CHECK_VALUE || esd_buf[3] != (u8)GTP_ESD_CHECK_VALUE) {
