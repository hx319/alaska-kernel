...
static void ilitek_report_touch_event(struct touch_data *touch, void *_private)
{
  ...
  if((touch->cnt) && (ts->double_wakeup != 0) && (ts->suspend_flag != 0))
  {
    if(ts->suspend_flag == 1)
    {
...
//        printk("gtp KEY_POWER\n");
    ...
    if(ts->revert_x)
      touch->finger[i].x = x_max - touch->finger[i].x + x_min;

    if(ts->revert_y)
      touch->finger[i].y = y_max - touch->finger[i].y + y_min;
...
static void ilitek_report_pen_event(struct pen_data *pen, void *_private)
{
  ...
  if(ts->revert_x)
    pen->pen.x = x_max - pen->pen.x + x_min;

  if(ts->revert_y)
    pen->pen.y = y_max - pen->pen.y + y_min;
...
void ilitek_suspend(void)
{
  ...
  {
    ts->suspend_flag = 1;
    dev_info(&((struct i2c_client *)ts->client)->dev, "double_wakeup suspend\n");
    return;
  }
	ts->esd_skip = true;
...
void ilitek_resume(void)
{
  ...
  {
    ts->suspend_flag = 0;
    dev_info(&((struct i2c_client *)ts->client)->dev, "double_wakeup resume\n");
    return;
  }
	if (ts->operation_protection || atomic_read(&ts->firmware_updating)) {
		TP_MSG(NULL, "operation_protection or firmware_updating return\n");
...
int ilitek_main_probe(void *client, struct device *device)
{
  ...
  ts->swap_x2y = 0;
  of_property_read_u32(ts->device->of_node, "ilitek,swap-x2y", &ts->swap_x2y);
  if (ts->swap_x2y)
    dev_info(&((struct i2c_client *)ts->client)->dev, "swap-x2y enabled\n");

  ts->revert_x = 0;
  of_property_read_u32(ts->device->of_node, "ilitek,revert-x", &ts->revert_x);
  if (ts->revert_x)
    dev_info(&((struct i2c_client *)ts->client)->dev, "revert-x enabled\n");

  ts->revert_y = 0;
  of_property_read_u32(ts->device->of_node, "ilitek,revert-y", &ts->revert_y);
  if (ts->revert_y)
    dev_info(&((struct i2c_client *)ts->client)->dev, "revert-y enabled\n");

  ts->suspend_flag = 0;
  ts->double_wakeup = 0;
  of_property_read_u32(ts->device->of_node, "ilitek,double-wakeup", &ts->double_wakeup);
  if(ts->double_wakeup)
    dev_info(&((struct i2c_client *)ts->client)->dev, "double-wakeup enabled\n");
