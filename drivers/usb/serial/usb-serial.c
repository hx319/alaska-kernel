...
static int allocate_minors(struct usb_serial *serial, int num_ports)
{
	...
	for (i = 0; i < num_ports; ++i) {
		port = serial->port[i];
    if((le16_to_cpu(serial->dev->descriptor.idVendor) == 0x0403)    //ftdi
      || (le16_to_cpu(serial->dev->descriptor.idVendor) == 0x067B)  //pl2303
      || (le16_to_cpu(serial->dev->descriptor.idVendor) == 0x1A86)  //ch340
      || (le16_to_cpu(serial->dev->descriptor.idVendor) == 0x4348)  //ch340
      || (le16_to_cpu(serial->dev->descriptor.idVendor) == 0x10C4))  //cp210x
  	   minor = idr_alloc(&serial_minors, port, 10, 0, GFP_KERNEL);
  	else
		  minor = idr_alloc(&serial_minors, port, 0, USB_SERIAL_TTY_MINORS, GFP_KERNEL);
		if (minor < 0)
			goto error;
		port->minor = minor;
		port->port_number = i;
