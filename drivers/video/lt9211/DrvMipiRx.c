...
SrtuctVideoTimingList resolution[]  = {
  {5,     5,    20,     720,     750,   110,   40,  220,  1280,   1650,   60},
...
void Drv_MipiRx_DesscPll_Set(void)
{
   ...
   printk("Mipi Rx PixClk: %d",g_stRxVidTiming.ulPclk_Khz);
...
void Drv_MipiRx_HsSettleSet(void)
{
    ...
    {
        printk("Set Mipi Rx Settle: 0x%02x", (g_stMipiRxVidTiming_Get.ucLane0SetNum - 5));
        HDMI_WriteI2C_Byte(0xff,0xd0);
        HDMI_WriteI2C_Byte(0x02,(g_stMipiRxVidTiming_Get.ucLane0SetNum - 5));
    }
    else
    {
        printk("Set Mipi Rx Settle: 0x03"); //mipi rx cts test need settle 0x0e
        HDMI_WriteI2C_Byte(0xff,0xd0);
...
void Drv_MipiRx_SotGet(void)
{
    ...
    printk("Sot Num = 0x%02x, 0x%02x, 0x%02x, 0x%02x", g_stMipiRxVidTiming_Get.ucLane0SetNum,g_stMipiRxVidTiming_Get.ucLane1SetNum,
                                                                    g_stMipiRxVidTiming_Get.ucLane2SetNum,g_stMipiRxVidTiming_Get.ucLane3SetNum);
    printk("Sot Dta = 0x%02x, 0x%02x, 0x%02x, 0x%02x", g_stMipiRxVidTiming_Get.ucLane0SotData,g_stMipiRxVidTiming_Get.ucLane1SotData,
...
void Drv_MipiRx_HactGet(void)
{
    ...
    switch (g_stMipiRxVidTiming_Get.ucFmt)
    {
        ...
        case 0x0e: //CSI-YUV422-10bpc
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 25/10; //wc = hact * 20bpp/8
        break;
        ...
        break; 
        case 0x04: //RGB10bpc
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 375/100; //wc = hact * 30bpp/8
        break;
        case 0x05: //RGB12bpc
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 45/10; //wc = hact * 36bpp/8
        break;
        ...
        break;
        case 0x08: //RGB6bpc
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc/ 225/100; //wc = hact * 18bpp/8
        break;
        ...
        break;
        case 0x0c: //RAW10
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 125/100; //wc = hact * 10bpp/8
        break;
        case 0x0d: //RAW12
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 15/10; //wc = hact * 12bpp/8
        break;
        default: 
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 3; //wc = hact * 24bpp/8
        break;
...
u8 Drv_MipiRx_VidTiming_Get(void)
{
    ...
    {
        printk("RX No Video Get");
        return FAIL;
    }
    else
    {
       printk("hact = %d",g_stMipiRxVidTiming_Get.usHact);
       printk("vact = %d",g_stMipiRxVidTiming_Get.usVact);    
       printk("fmt = 0x%02x", g_stMipiRxVidTiming_Get.ucFmt);
       printk("pa_lpn = 0x%02x", g_stMipiRxVidTiming_Get.ucPa_Lpn);
        return SUCCESS;
    }
}
...
{	
    ...
    for (uci = 0; uci < ucResolutionnum; uci++)
    {
            ...
            ( g_stMipiRxVidTiming_Get.usVact == resolution[uci].usVact ))
        {
           ...
           printk("FrameRate = %d", g_stMipiRxVidTiming_Get.ucFrameRate);
               ...
               (g_stMipiRxVidTiming_Get.ucFrameRate < (resolution[uci].ucFrameRate + 3)))
            {
               ...
               printk("Video Timing Set %dx%d_%d",g_stRxVidTiming.usHact,g_stRxVidTiming.usVact,g_stMipiRxVidTiming_Get.ucFrameRate);
                rtn = TRUE;
                break;
            }
        }
...
u8 Drv_MipiRx_VidFmt_Get(IN u8 VidFmt)
{
   ...
   printk("MipiRx Input Format: %s",g_szStrRxFormat[VidFmt]);
    return ucRxVidFmt;
}
...
void Drv_MipiRx_VidChkDebug(void)
{
    ...
    printk("mipi input hfp, hs, hbp, hact, htotal = %d %d %d %d %d",ushfp, ushs, ushbp, ushact, ushtotal);
	printk("mipi input vfp, vs, vbp, vact, vtotal = %d %d %d %d %d",usvfp, usvs, usvbp, usvact, usvtotal);
}
