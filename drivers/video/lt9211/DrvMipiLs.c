...
u8 Drv_MipiLsClk_Change(void)
{
    ...
    if (ulPixClk_New == 0)
    {
        printk("MIPI Repeater Level shift Rx Clk Lost");
        return TRUE;
    }
...
{
    printk("MIPI Repeater level shift Config");
...
void Drv_MipiLsTx_PllSet(void)
{
    ...
    printk( "ulPixClk: %d, ulMipiTXPhyClk: %d",ulPixClk, ulMipiTXPhyClk);
    ...
    printk("ucPreDiv :0x%02x, ucSericlkDiv :0x%02x, ucDivSet :0x%02x",ucPreDiv, ucSericlkDiv, ucDivSet);
...
void Drv_MipiLsBta_Set(void)
{
    ...
    printk("MIPI Repeater Level Shift Output");
}
...
void Drv_MipiLsClk_Check(void)
{
    ...
    ulTxByteClk = Drv_System_FmClkGet(AD_MLTX_READ_CLK);
    printk( "RX byte Clk: %d, Tx byte Clk: %d",ulRxByteClk, ulTxByteClk);
}
