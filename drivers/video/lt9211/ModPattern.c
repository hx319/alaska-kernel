...
#endif
 #if TX_VID_PATTERN_SEL == PTN_480x272_60
struct ChipRxVidTiming ptn_timing   = {4, 4, 43,480, 531,  4,  4,  12,  272,  292  , 9000};
#endif
...
struct video_pattern_timing pattern_480x272_60Hz = {4, 4, 43,480, 531,  4,  4,  12,  272,  292,    60};
...
void Mod_VidChkDebug_ForPtn(void)
{
    ...
    printk("ucframerate: %d",ucframerate);
	HDMI_WriteI2C_Byte(0xff,0x86);
...
void Mod_MipiTxpll_ForPtn(struct ChipRxVidTiming *ptn_timing)
{
    ...
    printk( "ulMipiDataRate: %d, ulMpiTXPhyClk: %d, ucSericlkDiv: %d, ucPreDiv: %d, ucDivSet: %d",
    ...
    {
        printk("LT9211C tx pll lock");
    }
    else
    {
        printk("LT9211C tx pll unlocked");
    }
 	
}


void Mod_MipiTxPhy_ForPtn(void)
{
    HDMI_WriteI2C_Byte(0xff,0x82);
...
void Mod_MipiTx_PhyTimingParaSet_ForPtn(void)
{
    u8 ucClkZero;      
    u8 ucClkPre = 0x02;       
    u8 ucClkPost;      
    ...
    printk("ulMipiDataRate: %d, ulrdbyteclk: %d",ulMipiDataRate, ulrdbyteclk);
    ...
    printk( "ck_post (0xD4A9) = 0x%02x", ucClkPost);
    printk( "ck_zero (0xD4A7) = 0x%02x", ucClkZero);
    printk( "hs_lpx  (0xD4A4) = 0x%02x", ucHsLpx);
    printk( "hs_prep (0xD4A5) = 0x%02x", ucHsPrep);
    printk( "hs_trail(0xD4A6) = 0x%02x", ucHsTrail);
    printk( "hs_rqst (0xD48A) = 0x%02x", ucHsRqStPre);
...
void Mod_MipiTxDig_Set_ForPtn(void)
{
    HDMI_WriteI2C_Byte(0xff,0xd4);
   ...
   HDMI_WriteI2C_Byte(0x82,0x00);
    ...
    printk("readdly = 0x%02x%02x",(HDMI_ReadI2C_Byte(0x82)), (HDMI_ReadI2C_Byte(0x83)));
...
void Mod_MipiTxPattern_Out(void)
{
    printk("*************LT9211C MIPITX Pattern Config*************");
    Mod_SystemInt();
    ...
    printk("*************LT9211C MIPITX Vid Pattern Out************");
...
//        ulLvdsTXPhyClk = ulDessc_Pix_Clk * 3.5;
   ...
   // ucDivSet = (u8)((ulLvdsTXPhyClk * ucSericlkDiv) / (pclk_khz / ucPreDiv));
    ...
    HDMI_WriteI2C_Byte(0x35,ucDivSet);
   // printk("ulPclk_Khz: %d, ucPreDiv: %d, ucSericlkDiv: %d, ucPixClkDiv: %f, ucDivSet: %d",pclk_khz, ucPreDiv, ucSericlkDiv, ucPixClkDiv, ucDivSet);

    printk("temp is: 0x%02x", temp);
    ...
    HDMI_WriteI2C_Byte(0x19,0xd4); //ssc_prd
        ...
        printk("LVDS Output SSC 1920x1080 30k2%");
        ...
        HDMI_WriteI2C_Byte(0x20,0x00);
        HDMI_WriteI2C_Byte(0x21,0x00);
        printk("LVDS Output SSC 3840x2160 30k2%");
    #endif
...
{  
   __maybe_unused u16 vss = 0;
   __maybe_unused u16 eav = 0;
   __maybe_unused u16 sav = 0;
...
void Mod_LvdsTxPattern_Out(void)
{
    printk("*************LT9211C LVDSTX Pattern Config*************");
    Mod_SystemInt();
	...
	Mod_VidChkDebug_ForPtn();
    printk("*************LT9211C LVDSTX Vid Pattern Out************");
}
...
void Mod_TtlTxHalfBit_ForPtn(void)
{
...
void Mod_TtlTxSyncInter_ForPtn(void)
{
...
void Mod_TtlTxColorSpace_ForPtn(void)
{
...
void Mod_TtlTxDdrMode_ForPtn(void)
{
    #if TTLTX_MODE == SDR
...
void Mod_TtlTxPhy_ForPtn(void)
{
    ...
    HDMI_WriteI2C_Byte(0x6D,0xff);
            ...
            printk("DDR Half Clock Mode:    Enable");
        #endif
            ...
            HDMI_WriteI2C_Byte(0x63,0x2b); //dll release
            HDMI_WriteI2C_Byte(0x63,0x2f); //phdly_en on
            printk("DDR Half Width Mode:    Enable");
        #endif
    #else
        ...
        HDMI_WriteI2C_Byte(0x2b,HDMI_ReadI2C_Byte(0x2b) | 0x01); //dessc double pix clock output
        printk("Half Bit Mode:    Enable");
    #endif
}


void Mod_TtlTxDig_ForPtn(void)
{
    HDMI_WriteI2C_Byte(0xff,0x86);
    ...
    HDMI_WriteI2C_Byte(0xaf,0x3f);
            ...
            printk("DDR Trans Mode:    High 12Bit First");
            #else
            HDMI_WriteI2C_Byte(0xaf,HDMI_ReadI2C_Byte(0xaf) & 0xbf); //low 12 Bit first
            printk("DDR Trans Mode:    Low 12Bit First");
            #endif
        #endif
    #endif
...
void Mod_TtlTxMapping_ForPtn(void)
{
    HDMI_WriteI2C_Byte(0xff,0x86);
    ...
    switch(TTLTX_OUTPUT_MODE)
    {
        case OUTPUT_RGB888:
            ...
            #endif
            printk("TTLTx Output Mode:    RGB888");
            break;
        ...
        case OUTPUT_RGB666:
            ...
            //RGB666 swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x70);
            #endif
            printk("TTLTx Output Mode:    RGB666");
            break;
        ...
        case OUTPUT_RGB565:
            ...
            //RGB565 case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x70);
            #endif
            printk("TTLTx Output Mode:    RGB565");
            break;
        ...
        case OUTPUT_YCBCR444:
            ...
            //RGB high/low swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x74);
            #endif
            printk("TTLTx Output Mode:    YCBCR444");
            break;
        
        case OUTPUT_YCBCR422_8BIT:
            printk("TTLTx Output Mode:    YCBCR422_8BIT");
            break;
        
        case OUTPUT_YCBCR422_10BIT:
            printk("TTLTx Output Mode:    YCBCR422_10BIT");
            break;
        
        case OUTPUT_YCBCR422_12BIT:
            printk("TTLTx Output Mode:    YCBCR422_12BIT");
            break;
        ...
        case OUTPUT_YCBCR422_16BIT:
            ...
            //RGB high/low swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x72);
            #endif
            printk("TTLTx Output Mode:    YCBCR422_16BIT");
            break;
        ...
        case OUTPUT_YCBCR422_20BIT:
            ...
            //RGB swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x40);
            #endif
            printk("TTLTx Output Mode:    YCBCR422_20BIT");
            break;
        ...
        case OUTPUT_YCBCR422_24BIT:
            ...
            //RGB swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x40);
            #endif
            printk("TTLTx Output Mode:    YCBCR422_24BIT");
            break;
        ...
        case OUTPUT_BT656_8BIT:
            ...
            #endif
            printk("TTLTx Output Mode:    BT656_8BIT");
            break;
        ...
        case OUTPUT_BT656_10BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            printk("TTLTx Output Mode:    BT656_10BIT");
            break;
        ...
        case OUTPUT_BT656_12BIT:
            ...
            #endif
            printk("TTLTx Output Mode:    BT656_12BIT");
            break;
        ...
        case OUTPUT_BT1120_8BIT:
            //RGB swap case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x40);
            #endif
            printk("TTLTx Output Mode:    BT1120_8BIT");
            break;
        ...
        case OUTPUT_BT1120_10BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            printk("TTLTx Output Mode:    BT1120_10BIT");
            break;
        ...
        case OUTPUT_BT1120_12BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x01);
            #endif
            printk("TTLTx Output Mode:    BT1120_12BIT");
            break;
        ...
        case OUTPUT_BTA_T1004_16BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            printk("TTLTx Output Mode:    BTA_T1004_16BIT");
            break;
        ...
        case OUTPUT_BTA_T1004_20BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x40);
            #endif
            printk("TTLTx Output Mode:    BTA_T1004_20BIT");
            break;
        ...
        case OUTPUT_BTA_T1004_24BIT:
            //YC swap case 
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            printk("TTLTx Output Mode:    BTA_T1004_24BIT");
            break;
        ...
        case OUTPUT_BT1120_16BIT: 
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            printk("TTLTx Output Mode:    BT1120_16BIT");
            break;
        ...
        case OUTPUT_BT1120_20BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x40);
            #endif
            printk("TTLTx Output Mode:    BT1120_20BIT");
            break;
        ...
        case OUTPUT_BT1120_24BIT:
            //YC swap case 
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            printk("TTLTx Output Mode:    BT1120_24BIT");
            break;
    }
}


void Mod_CscSet_ForTtlPtn(void)
{
    #if TTLTX_COLORSPACE == RGB
        printk("Color Format:    RGB");
    #elif TTLTX_COLORSPACE == YUV444
        HDMI_WriteI2C_Byte(0xff,0x86);
        HDMI_WriteI2C_Byte(0x85,0x10);
        printk("Color Format:    YUV444");
        ...
        printk("Color Format:    YUV422");
    #endif
}


void Mod_TtlTx_BtTiming_Set(void)
{
    u16 vss,hss;
...
void Mod_TtlTxPhaseSet_ForPtn(void)
{
    ...
    pclk_khz = Drv_System_FmClkGet(AD_DESSCPLL_PIX_CLK);
            ...
            #endif
        #endif
        #if TTLTX_HALF_WIDTH_MODE == ENABLED//DPIÄ£Ê½
    ...
    printk("pix clk = %d kHz",pclk_khz);
    printk("out clk = %d kHz",io_dclk_khz);
    printk("DLL phase set: 0x%02x", ucphase);
...
void Mod_TtlTxPattern_Out(void)
{
    printk("*************LT9211C TTLTX Pattern Config*************");
    Mod_SystemInt();
    ...
    HDMI_WriteI2C_Byte(0x0d,0x7d); //ttl tx reset
    HDMI_WriteI2C_Byte(0x0d,0x7f); //ttl tx release
    printk("*************LT9211C TTLTX Vid Pattern Out************");
}
