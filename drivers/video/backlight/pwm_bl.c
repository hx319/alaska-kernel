...
static int pwm_backlight_update_status(struct backlight_device *bl)
{
	...
	if (brightness > 0) {
		pwm_get_state(pb->pwm, &state);
//    printk("set bl %d, get bl %d\n", brightness, brightness * (pb->pwm_max_limit - pb->pwm_min_limit) / bl->props.max_brightness + pb->pwm_min_limit);
    brightness = brightness * (pb->pwm_max_limit - pb->pwm_min_limit) / bl->props.max_brightness + pb->pwm_min_limit;
...
static int pwm_backlight_probe(struct platform_device *pdev)
{
	...
	if(of_property_read_u32(node, "pwm_min_limit", &pb->pwm_min_limit) < 0)
	  pb->pwm_min_limit = 28;
	if(of_property_read_u32(node, "pwm_max_limit", &pb->pwm_max_limit) < 0)
	  pb->pwm_max_limit = data->max_brightness;
