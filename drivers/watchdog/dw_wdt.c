...
static int watchdog_run_thread(void *parg);
static void __iomem *reboot_wdt_base;
static struct clk		*wdt_clk;
extern struct device *pet_dev;
static long r_watchdog_state = 3;

//#define DBG_WATCHDOG(format,args...)  printk("[r_watchdog] "format,##args)
#define DBG_WATCHDOG(format,args...)
...
void stop_r_watchdog(void)
{
  u32 reg;
  if(watchdog_task)
  {
    reg = readl(reboot_wdt_base);
    reg &= 0xFE;
    writel(reg, reboot_wdt_base);
    kthread_stop(watchdog_task);
    watchdog_task = NULL;
    DBG_WATCHDOG("kernel wdt thread end\n");
  }
}

void start_r_watchdog(void)
{
  u32 reg;
  if(watchdog_task == NULL)
  {
    /* SetTimeOut */
    writel(0x0D, reboot_wdt_base + 0x04);

    /* clear cont */
    writel(0x76, reboot_wdt_base + 0x0C);

    /* Disable interrupt mode, Enable watchdog */
    reg = readl(reboot_wdt_base);
    reg &= 0xFD;
    reg |= 0x01;
    writel(reg, reboot_wdt_base);
    ...
    DBG_WATCHDOG("kernel wdt thread run\n");
  }
}
...
static ssize_t watch_dog_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
  if(kstrtoul(buf, 0, &r_watchdog_state))
    return 0;
  DBG_WATCHDOG("set r_watchdog_state %ld\n", r_watchdog_state);
  switch(r_watchdog_state)
  {
    case 0://关闭外部喂狗，自动切换到内核线程喂狗
    case 4://使能看门狗，自动开启内核喂狗线程，看门狗有效，内核自动喂狗
    default:
      clk_prepare_enable(wdt_clk);
      start_r_watchdog();
      break;
    case 1://开启外部喂狗，自动关闭内核喂狗线程，看门狗依然有效，需要进行外部喂狗操作，否则会重启
      if(watchdog_task)
      {
        kthread_stop(watchdog_task);
        watchdog_task = NULL;
    ...
    case 2://外部程序喂狗
        writel(0x76, reboot_wdt_base + 0x0C);
      break;
    case 3://禁用看门狗，自动关闭内核喂狗线程，看门狗无效，无需进行喂狗
      stop_r_watchdog();
...
static int watchdog_run_thread(void *parg)
{
  ...
  while (1)
  {
    if(kthread_should_stop()) {
      DBG_WATCHDOG("stop kick watch_dog\n");
      break;
    }
    //喂狗
    DBG_WATCHDOG("kick watch_dog\n");
    writel(0x76, reboot_wdt_base + 0x0C);
...
static int dw_wdt_suspend(struct device *dev)
{
  ...
  if(watchdog_task) {
    ...
    reg &= 0xFE;
    writel(reg, reboot_wdt_base);
    DBG_WATCHDOG("sunxi_wdt_suspend\n");
  }
...
static int dw_wdt_drv_probe(struct platform_device *pdev)
{
  ...
  reboot_wdt_base = dw_wdt->regs;
  wdt_clk = dw_wdt->clk;
  start_r_watchdog();
  r_watchdog_state = 0;
  device_create_file(pet_dev, &dev_attr_watch_dog);
	return 0;
