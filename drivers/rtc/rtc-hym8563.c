...
int hym8563_get_state(void)
{
  unsigned char buf;
  if(this_client == NULL)
    return 0;

  if(i2c_smbus_read_i2c_block_data(this_client, HYM8563_CTL2, 1, &buf) >= 0)
  {
//    printk("hym8563_get_state %x\n", buf);
    if (buf & HYM8563_CTL2_AF)
    {
		  i2c_smbus_write_byte_data(this_client, HYM8563_CTL2,(buf & ~HYM8563_CTL2_AF));
      return 1;
    }
  }
  return 0;
			 ...
			 const struct i2c_device_id *id)
{
  ...
  do
  {
    ...
    if(ret == 0)
      break;
    msleep(100);
    printk("hym8563_init_device retry %d\n", ++retry);
  }while(retry < 10);
	if (ret) {
...
int hym8563_get_time(struct rtc_time *tm)
{
  if(this_client !=NULL)
    return hym8563_rtc_read_time(&this_client->dev, tm);
  else
    return -1;
}

int hym8563_set_time(struct rtc_time *tm)
{
  if(this_client !=NULL)
    return hym8563_rtc_set_time(&this_client->dev, tm);
  else
    return -1;
}
